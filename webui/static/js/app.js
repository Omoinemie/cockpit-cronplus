...
(function () {
    ...
    var LOGS_FILE = '/opt/cronplus/logs/logs.json';
    ...
    function clearDaemonLogView() {
        daemonLogLines = [];
        var viewer = $('#daemonLogViewer');
        if (viewer) viewer.innerHTML = '<div class="daemon-log-empty">' + I18n.t('daemon.noLogs') + '</div>';
        var statusEl = $('#daemonLogStatus');
    ...
    async function loadLogs() {
        try {
            logs = await Utils.shellReadJson(LOGS_FILE);
        } catch (e) { logs = []; }
    }
    ...
    function editTask(index) {
        ...
        } else if ((task.schedule || '').startsWith('@')) {
            $$('.preset-btn').forEach(function (b) { b.classList.remove('active'); });
            var mp2 = $('.preset-btn[data-preset="' + task.schedule + '"]');
            if (mp2) mp2.classList.add('active');
            $('#cronSec').value = '0';
            $('#cronMin').value = task.schedule;
            $('#cronHour').value = ''; $('#cronDay').value = ''; $('#cronMonth').value = ''; $('#cronDow').value = '';
    ...
    function execCleanup() {
        ...
        var removeSet = new Set(toRemove.map(function (l) { return logs.indexOf(l); }));
        var newLogs = logs.filter(function (_, i) { return !removeSet.has(i); });

        Utils.shellWriteJson(LOGS_FILE, newLogs).then(function () {
            logs = newLogs;
            renderLogs();
            renderTasks();
            closeCleanupModal();
    ...
    function bindEvents() {
        ...
        $('#btnDaemonClear').addEventListener('click', clearDaemonLogView);
